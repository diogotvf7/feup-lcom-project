...
int (set_data_reporting)(bool enable) 
{
  ...
  while (attempts_left-- && out != ACK) {
    if (kbc_write(KBC_CMD_REG, WRITE_BYTE_MOUSE) != OK) return EXIT_FAILURE;
    if (kbc_write(KBC_IN_BUF, enable ? ENABLE_DATA_REP : DISABLE_DATA_REP) != OK) return EXIT_FAILURE;
